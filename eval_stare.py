#!/usr/bin/env python3
"""
STARE deep-supervision lambda grid evaluation.

Examples:
  python eval_stare.py --models_dir ./models/stare --data_dir ./data/STARE --results_dir ./results/stare
  python eval_stare.py --models_dir ./models/stare --data_dir ./data/STARE --results_dir ./results/stare --optimize_threshold
"""
...
def _pair_images_and_masks(im_dir: str, mask_dir: str):
    ...
    if not im_files:
        raise RuntimeError(f"No test images found: {im_dir}")
...
class StareTestDataset(torch.utils.data.Dataset):
    def __init__(self, data_dir: str):
        ...
        if im_dir is None or lb_dir is None:
            raise RuntimeError(f"STARE/test missing im or label folder: {test_root}")
        self.pairs = _pair_images_and_masks(im_dir, lb_dir)
...
def get_test_loader_stare(data_dir: str, batch_size=1, num_workers=4):
    dataset = StareTestDataset(data_dir)
...
def evaluate_lambda_grid(models_dir, data_dir, device, args):
    ...
    test_loader = get_test_loader_stare(
        data_dir,
        batch_size=1,
...
def plot_lambda_f1_curve(summary_df, save_dir):
    ...
    ax.set_title("STARE: lambda grid")
    ax.grid(True)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="STARE lambda grid evaluation")

    parser.add_argument("--models_dir", type=str, default="./models/stare", help="Checkpoint root.")
    parser.add_argument("--data_dir", type=str, default="./data/STARE", help="Dataset root.")
    parser.add_argument("--results_dir", type=str, default="./results/stare", help="Result directory.")
    ...
    print("\n" + "=" * 60)
    print("STARE evaluation finished")
